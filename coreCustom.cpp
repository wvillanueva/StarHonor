...
volatile uint8_t *ArduboyCoreCustom::csport, *ArduboyCoreCustom::dcport;
uint8_t ArduboyCoreCustom::cspinmask, ArduboyCoreCustom::dcpinmask;
...
ArduboyCoreCustom::ArduboyCoreCustom() {}

void ArduboyCoreCustom::boot()
{
...
void ArduboyCoreCustom::setCPUSpeed8MHz()
{
...
void ArduboyCoreCustom::bootPins()
{
  uint8_t pin, mode;
...
void ArduboyCoreCustom::bootOLED()
{
...
void ArduboyCoreCustom::LCDDataMode()
{
  *dcport |= dcpinmask;
  *csport &= ~cspinmask;
}

void ArduboyCoreCustom::LCDCommandMode()
{
  *csport |= cspinmask;
  *dcport &= ~dcpinmask;
  *csport &= ~cspinmask;
...
void ArduboyCoreCustom::safeMode()
{
...
void ArduboyCoreCustom::idle()
{
  set_sleep_mode(SLEEP_MODE_IDLE);
  sleep_mode();
}

void ArduboyCoreCustom::bootPowerSaving()
{
  power_adc_disable();
...
uint8_t ArduboyCoreCustom::width() { return WIDTH; }

uint8_t ArduboyCoreCustom::height() { return HEIGHT; }


/* Drawing */

void ArduboyCoreCustom::paint8Pixels(uint8_t pixels)
{
  SPI.transfer(pixels);
}

void ArduboyCoreCustom::paintScreen(const unsigned char *image)
{
...
void ArduboyCoreCustom::paintScreen(unsigned char image[])
{
  for (int i = 0; i < (HEIGHT*WIDTH)/8; i++)
  {
...
void ArduboyCoreCustom::blank()
{
  for (int i = 0; i < (HEIGHT*WIDTH)/8; i++)
    SPI.transfer(0x00);
}

void ArduboyCoreCustom::sendLCDCommand(uint8_t command)
{
  LCDCommandMode();
...
void ArduboyCoreCustom::invert(bool inverse)
{
...
void ArduboyCoreCustom::allPixelsOn(bool on)
{
  sendLCDCommand(on ? OLED_ALL_PIXELS_ON : OLED_PIXELS_FROM_RAM);
}

// flip the display vertically or set to normal
void ArduboyCoreCustom::flipVertical(bool flipped)
{
  sendLCDCommand(flipped ? OLED_VERTICAL_FLIPPED : OLED_VERTICAL_NORMAL);
}

// flip the display horizontally or set to normal
void ArduboyCoreCustom::flipHorizontal(bool flipped)
{
...
void ArduboyCoreCustom::setRGBled(uint8_t red, uint8_t green, uint8_t blue)
{
...
uint8_t ArduboyCoreCustom::getInput()
{
  return buttonsState();
}


uint8_t ArduboyCoreCustom::buttonsState()
{
  uint8_t buttons;
